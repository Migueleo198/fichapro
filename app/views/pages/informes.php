...
<form method="GET" action="<?= url('informes') ?>" class="card" style="padding:1rem 1.25rem;margin-bottom:1.5rem;display:flex;gap:.75rem;align-items:flex-end;flex-wrap:wrap;">
    <div style="flex:1;min-width:140px;"><label class="lbl">Desde</label><input type="date" name="desde" value="<?= e($desde) ?>" class="inp"></div>
    <div style="flex:1;min-width:140px;"><label class="lbl">Hasta</label><input type="date" name="hasta" value="<?= e($hasta) ?>" class="inp"></div>
    <button class="btn btn-teal"><i class="bi bi-search"></i> Generar</button>
</form>
...
<div style="display:grid;grid-template-columns:3fr 2fr;gap:1.25rem;">
    <div class="card">
        ...
        <table class="tbl">
            ...
            <tr>
                <td style="font-weight:800;color:#0A2E4E;"><?= e($r['nombre'].' '.$r['apellidos']) ?></td>
                <td><?= (int)$r['dias'] ?></td>
        ...
        <div style="padding:1.25rem;">
            ...
            <div style="display:flex;align-items:flex-end;gap:3px;height:140px;">
                ...
                <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;min-width:0;">
                    <div title="<?= fechaLarga($d['fecha']) ?>: <?= horasLegibles($d['horas']) ?>" style="width:100%;border-radius:4px 4px 0 0;background:#3EC6C1;height:<?= $h ?>px;cursor:help;"></div>
                </div>
                <?php endforeach; ?>
            </div>
            <div style="display:flex;gap:3px;margin-top:.35rem;">
                <?php foreach ($porDia as $d): ?><div style="flex:1;text-align:center;min-width:0;"><span style="font-size:8px;color:#7a9cb0;font-weight:700;"><?= date('d/m',strtotime($d['fecha'])) ?></span></div><?php endforeach; ?>
            </div>
