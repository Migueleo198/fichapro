...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;}
        ...
        .btn-main:hover{background:#155380 !important;box-shadow:0 6px 18px rgba(26,100,156,.32) !important;}
...
<div style="width:100%;max-width:400px;">
    <div style="text-align:center;margin-bottom:1.6rem;">
        <div style="width:3.4rem;height:3.4rem;border-radius:15px;background:linear-gradient(135deg,#1A649C,#1E3A8A);display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:1.55rem;box-shadow:0 10px 24px rgba(26,100,156,.35);"><i class="bi bi-key-fill"></i></div>
        ...
        <p style="font-size:.82rem;font-weight:500;color:var(--muted);margin:0;">Recuperación de contraseña</p>
    </div>
        ...
        <div style="padding:1.8rem 1.9rem 1.9rem;">
            ...
            <?php if (!empty($ok)): ?>
                <div style="text-align:center;">
                    <i class="bi bi-envelope-check" style="font-size:2.2rem;color:var(--green);"></i>
                    <h2 style="font-size:1.02rem;font-weight:700;margin:.6rem 0 .3rem;">Revisa tu correo</h2>
                    <p style="font-size:.85rem;color:var(--mid);margin:0 0 1.3rem;line-height:1.5;">
                        Si ese correo está registrado, te hemos enviado un enlace para restablecer tu contraseña. El enlace caduca en 1 hora.
                    </p>
                    <a href="<?= url('login') ?>" class="link"><i class="bi bi-arrow-left"></i> Volver al inicio de sesión</a>
                </div>
            <?php else: ?>
                <h2 style="text-align:center;font-size:1.02rem;font-weight:700;color:var(--ink);margin:0 0 .25rem;">¿Olvidaste tu contraseña?</h2>
                <p style="text-align:center;font-size:.82rem;font-weight:500;color:var(--muted);margin:0 0 1.5rem;">Introduce tu correo y te enviaremos un enlace para restablecerla</p>

                <?php if (($err ?? null) === 'email'): ?>
                ...
                <form method="POST" action="<?= url('login/enviarRecuperacion') ?>">
                    <div style="margin-bottom:1.3rem;">
                        <label style="display:block;font-size:.78rem;font-weight:600;color:var(--mid);margin-bottom:.4rem;">Correo electrónico</label>
                        <div class="iw" style="display:flex;align-items:center;gap:.65rem;border:1px solid var(--border);border-radius:10px;padding:.65rem .9rem;background:#fff;transition:all .16s;">
                            <i class="bi bi-envelope" style="color:var(--muted);font-size:1.05rem;"></i>
                            <input type="email" name="email" placeholder="tu@correo.com" autofocus required class="lg-input"
                    ...
                    <button type="submit" class="btn-main" style="width:100%;padding:.85rem;border-radius:10px;border:none;background:var(--blue);color:#fff;font-family:inherit;font-size:.92rem;font-weight:700;cursor:pointer;letter-spacing:-0.01em;box-shadow:0 4px 14px rgba(26,100,156,.3);">
                        <i class="bi bi-send" style="margin-right:.4rem;"></i>Enviar enlace
                    </button>
                </form>
